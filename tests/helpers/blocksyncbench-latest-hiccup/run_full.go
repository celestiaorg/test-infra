...
func RunFullNode(runenv *runtime.RunEnv, initCtx *run.InitContext) error {
	...
	bridgeNode, bridgeNodes, err := common.GetBridgeNodes(ctx, syncclient, initCtx.GroupSeq, runenv.IntParam("bridge"))
	if err != nil {
		return err
	}
	...
	entryPointsMap, err := common.ParseFullNodeEntryPointsKey(runenv.StringParam("full-node-entrypoints"))
	if err != nil {
		return err
	}
