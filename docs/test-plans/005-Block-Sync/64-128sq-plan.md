...
3. 32 Full Nodes with each having 1 CPU and 8 Gb
...
Z Full nodes all connected to some or all of the existing Y bridges, and connected amongst themselves (_peerLimit is set to W_) such that full nodes are syncing latest blocks as they are being produced.
...
Z Full nodes all connected to some or all of the existing Y bridges, and connected amongst themselves (_peerLimit is set to W_) such that full nodes are syncing latest blocks as they are being produced, with random full nodes being disconnecting from Bridge node a given random `network-partition-height`.
